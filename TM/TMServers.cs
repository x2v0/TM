...
// Добавление нового сервера в набор
      public virtual int AddServer(string name, string ip, int port, EServerType type)
      {
         ...
         if (type == EServerType.TMC)
         {
            ...
            ds.ConnectType = EConnectType.TMPROTOCOL;
            ds.ConnectAvailable = EConnectType.TMPROTOCOL;
         }
         // для сервера вакуума
         else if( type == EServerType.DACADC )
         {
            ...
            ds.ConnectType = EConnectType.LNKLIB;
            ds.ConnectAvailable = EConnectType.LNKLIB;
...
int StopUpdateTimer()
{
   ...
   return -1; // не был запущен
      ...
      int Servers_StartUpdateTimer( Server *ds, double period,  CtrlCallbackPtr fupdate )
      {
      ds.F_update = fupdate;

      // если таймер уже был взведен - ликвидируем
      Servers_StopUpdateTimer( ds );
      ...
      //int Servers_GetServerStatus( Server *ds )
      //{
      //return ds.DevStatus;
      //}
      ...
      VC_Control *Servers_AddVC_Control( Server *ds, int type, int masterCID, int CID, int num, int count )
      {
      // добавляем новый
      ...
      int Servers_DelVC_Control( Server *ds, int num )
      {
      ...
      int Servers_Del( int num )
      {
      Server *ds;         // сервер

      if( num <0 || num>=ServerCount ) return -1;
      ds = Servers[num];  

      // подчищаем список
      for(int i=num; i<ServerCount-1; i++)
      {
      ...
      Servers_DisconnectFromServer(ds);  
      ...
      int DeleteDataServer(Server ds)
      {
      foreach(var datasever in DataServers)
      {
         if( Servers[i] == ds ) 
            return Servers_Del(i);
      }
      return -1;
      }
      ...
      Server GetServer(int num)
      {
      if( num <0 || num>=ServerCount ) return NULL;

      return Servers[num];   
      }

      //поиск сервера по контролу его или датчика
      Server *Servers_GetServerByCntlId(int id)
      {
      Server *ds;
      ...
      void ClientTCP_ParseIncommingPacketTM(Server *ds, int handle)
      {
      Packet *p;
      ...
      // вызывается из ClientTCPCB
      void ClientTCP_ParseIncommingPacketNTM(Server *ds, int handle)
      {
      Packet *p=NULL;
      unsigned char *pdata=NULL;
      ...
      void Servers_UpdateDevicesStatus(void)
      {
      for(int i=0; i<ServerCount; i++)
      {
         Server *ds = Servers[i];
      ...
      // вызывается из Servers_UpdateDevicesStatus
      // статус отображается в Server_ShowDevicesStatus
      void ChangeVacDevicesStatus( Server *ds )
      {
      ...
      int Servers_SendCustomToServer(Server *ds, void *data, size_t size)
      {
      if( ds == NULL ) return -1;
      ...
      int  Servers_GetNextAskNum( Server *ds )
      {
      ...
      int  Servers_SendTMCommand( Server *ds, int cmd )
      {
      int res;
      if( ds == NULL ) return -1;
      ...
      // default:
            //mprntf("send %d", cmd);
      res = SendCommandToServer( ds.type, ds.handle, cmd, DATA_LENGTH_0, DATA_STRUCT_NULL,  Servers_GetNextAskNum(ds), ds.TMSettings );  
      if( res != 0 )
      ...
      int  Servers_SendTMCommandD( Server *ds, int cmd, size_t sz, unsigned char *data  )
      {
      if( ds == NULL ) return -1;
      if( ds.ConnectType != SERVER_ConnectType_TMPROTOCOL ) return -2;

      SendCommandToServer( ds.type, ds.handle, cmd, sz, data,  Servers_GetNextAskNum(ds), ds.TMSettings ); 

      return 0;
      }

      // отправка данных на сервер по TM протоколу
      int  Servers_SendTMData( Server *ds, int cmd, size_t sz, unsigned char *data )
      {
      //unsigned char *data;
      //size_t sz;
      ...
      if( ds.F_SendTMDataPriv != NULL )
         ...
         return SendDataToServer( ds.type, ds.handle, cmd, sz, data, Servers_GetNextAskNum(ds), ds.TMSettings);
      }

      // отправка данных - текстовой строки - на сервер по TM протоколу
      int  Servers_SendTMInfo( Server *ds, int cmd, size_t sz, unsigned char *data )
      {
      if( ds == NULL ) return -1;
      if( ds.ConnectType != SERVER_ConnectType_TMPROTOCOL ) return -2;

      return SendInfoToServer( ds.type, ds.handle, cmd, sz, data, Servers_GetNextAskNum(ds), ds.TMSettings);
      }

      // отображаем цветом состояние датчиков.
      // статус меняется в ChangeTermometersStatus или ChangeXRayDevicesStatus
      void Servers_ShowServersDevicesStatus( void )
      {
      Server *ds;
      ...
      void Servers_ShowDevicesStatus( Server *ds )
      {
      ...
      // фиксируется при создании соединения в Servers_ConnectToServer  при типе соединения SERVER_ConnectType_TMPROTOCOL
      ...
      Server *ds;
       ...
       switch (event)                                                                             
           ...
           case TCP_DATAREADY: // сервер прислал данные
            ...
            ds = Servers_GetServerByHandle(handle);
      ...
      // возвращает Server_CONNECTED = 0 в случае успеха
      int ConnectToServer(Server ds)
      {
      ...
      else if( ds.ConnectType == SERVER_ConnectType_LNKLIB )
      {
         ...
         //sRq = Servers_GetServerNum( ds ); // для уникальности номера
         switch( ds.type )
         {
      ...
      else if(  ds.ConnectType == SERVER_ConnectType_IMITATION )  // имитация подключения к устройству - всегда успешное. Для программ-имитаторов
      {
         ...
         res = Server_CONNECTED; // 0 - всегда успех
      }
      ...
      // распечатка данных Server серверов в структуру-дерево
      int Servers_Info2Tree( int panel, int control, int n )
      {
      for(int i=0; i<ServerCount; i++)
      {
         n = Server_Info2Tree( Servers[i], panel, control, n );
      }
      return n;   
      }

      // распечатка данных Server сервера в структуру-дерево
      int Server_Info2Tree( Server *ds, int panel, int control, int n )
      {
      char buf[128];
      int h = n;
      ...
      if( ds != NULL )
      {
         ...
         sprintf( buf, "connection: %s", (Server_CONNECTED == Servers_IsConnected(ds))?"Connected":"Not connected" );
         InsertTreeItem (panel, control, VAL_CHILD, h, VAL_LAST, buf, NULL, NULL, n++);
      ...
      for(int i=0; i<ServerCount; i++)
      {
         ...
         if( Server_CONNECTED != Servers_IsConnected( ds ) && ds.NeedForReconnect )  // уже подключен или восстанавливать не требуется
         {

            // восстанавливаем подключение
            ds.OnProcessing = 1;

            res = Servers_ConnectToServer( ds );
            // если подключились - представимся подключенному серверу
            if( res == Server_CONNECTED && ds.ConnectType == SERVER_ConnectType_TMPROTOCOL )
            {
               sprintf(buf, "I'm %s client on %s", ds.name, configParams.appName ); 
               Servers_SendTMInfo( ds, Packet_INFO_MESSAGE, sizeof(buf), (unsigned char*)buf );
            }
      ...
      void Servers_DisconnectFromServer(Server ds)
      {
      if( ds == NULL ) return;
      if( ds.F_Disconnect ) 
      { 
      ...
      // результат Server_CONNECTED (0) - подключен
      int Servers_IsConnected( Server *ds )
      {
      if( ds == NULL ) return -1;
      ...
      {
         if( ds.handle >= 0 )
            return Server_CONNECTED; // подключен
      }
      else if(  ds.ConnectType == SERVER_ConnectType_LNKLIB  )
      {
         if( ds.lnk.sid >= 0 )
      ...
      // 0 - успех
      int  Servers_SetConnectType( Server *ds, int ConnectType )
      {
      if( ds == NULL ) return -1;

      if( Servers_IsConnected( ds ) )
         Servers_DisconnectFromServer( ds );

      // сравниваем с маской
      if( 0 == (ds.ConnectAvailable & ConnectType) )
      {
         mprntf_t("Connection type restricted for this type of Servers");
         return -1;
      }
      ...
      int Servers_GetServerNum( Server *ds )
      {
      for(int i=0; i<ServerCount; i++)
      {
         if( ds == Servers[i] )
      ...
      Server *Servers_GetServerByNum( int n )
      {
      if( n < 0 || n>= ServerCount ) return NULL;

      return Servers[n];
      }

      // расшифровка статуса возвращенного линклиб
      void Servers_ShowLnkLibSts( Server *ds, unsigned char rq_sts )     
      {
      ...
      int Servers_LinkLibCBND( PKT *rq )
      {
      int res = 0;
      return res;
      }
      ...
      // инициализируется в ( SendCommandToPFS ). Servers_SendCommandToHW
      static int Servers_LinkLibCB( PKT *rq, void *data )
      {
      // PFS_data *pfsd = NULL;  // данные сервера кресла
      Server *ds = NULL;     // сервер кресла
      DEV_DESCR *dds;
      int res = 0;
      //unsigned char sts;
      //char buf[128];
      ...
      if( rq.sts) {
         // Error code returned, no other valid data
         Servers_ShowLnkLibSts( ds, rq.sts );
         ds.DevStatus  = DEVICE_STATUS_CRACKED;
      ...
      // вызывается из [ SendCommandToPFS, Termo_SendCommandToDev ] . Servers_SendCommandToHW
      ...
      int Servers_SendCommandToDev( DEV_DESCR *dds, unsigned char command, double wait)
      {
      int res = 0;
      Server *ds = (Server *) dds.p_ds;

      if( !dds.configured ) return 0;

      res = Servers_SendCommandToHW( dds, command );
      if( res ) return res;
      ...
      int Servers_SetLL_NTF( DEV_DESCR *pdds, int cmd, int (* cbfnq)(PKT *) )
      {
      Server *ds = (Server *)pdds.p_ds;
      ...
      int Servers_SendCommandToHW( DEV_DESCR *pdds, int cmd )
      {
      int res = 0;
      Server *ds = (Server *)pdds.p_ds;
      ...
      if( Server_CONNECTED != Servers_IsConnected(ds) ) 
         return -1;
      ...
      if( ds.ConnectType == SERVER_ConnectType_LNKLIB )   // подключение по протоколу LNKLIB
      {
         ...
         ask_str.p_ds = ds;
         ask_str.asknum = Servers_GetNextAskNum( ds );;
         ask_str.p_dds = pdds;
         ...
         PktWrCBD( ds.lnk.RQ|FN_ANS|FN_WEV, &ask_str.sdev, (int)sizeof(ask_str.sdev), Servers_LinkLibCB, ask_str );
      // PktWrC( ds.lnk.RQ|FN_ANS|FN_NFY, &ask_str.sdev, (int)sizeof(ask_str.sdev), Servers_LinkLibNTFCB );
      //    PktWrCBD( ds.lnk.RQ|FN_ANS|FN_WEV, &ask_str.sdev, (int)sizeof(ask_str.sdev), Servers_LinkLibCB, ask_str ); // +
      //    PktWrC( ds.lnk.RQ|FN_ANS|FN_CYC, &ask_str.sdev, (int)sizeof(ask_str.sdev), Servers_LinkLibCBND );
      ...
      int SendCurrentConfig_PackConnectAsTag( unsigned char *data, int n, Server *ds )
      {
      int sz_pos;
      ...
      *val = b1 | (b2<<8)| (b3<<16) | (b4<<24);
      ...
      int ParseCfgPacket_GetLinkData( unsigned char *data, int n, Server *ds )
      {
      //n++; 
      unsigned char cnt = *(data+n++);
      int res = cnt+n;
      ...
      int  Servers_ParseServerType( char *buf )
      {
      int type;
      ...
      int  Servers_ParseConnectType( char *buf )
      {
      int type=0;
      ...
      int  Servers_PrintConnectType( Server *ds, char *buf )
      {
      if( ds.ConnectType == SERVER_ConnectType_LNKLIB )
      ...
      // таймер генерится в Servers_StartUpdateTimer( ds, 10.0, Servers_PingServiceTimerCB );
      // используется VisualControl: VC_AddServer
      int CVICALLBACK Servers_PingServiceTimerCB (int panel, int control, int event,
         void *callbackData, int eventData1, int eventData2)
      {
      Server *ds = (Server *) callbackData;
      //mprntf("Ping CB", ds.ip);
      if( Servers_IsConnected( ds ) == Server_CONNECTED )
      {
         if(0 != Servers_Ping(ds) )
            DisconnectFix( ds );
      }
      return 0;
      }
      ...
      // вызывается из Servers_PingServiceTimerCB
      // 0 - пинг успешен
      int Servers_Ping(void *pds)
      {
      int available;
      VC_Control *vcc;  
      char buf[1024];
